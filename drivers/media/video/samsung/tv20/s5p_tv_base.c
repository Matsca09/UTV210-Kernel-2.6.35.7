...
static int __devinit s5p_tv_probe(struct platform_device *pdev)
{
	...
	//hdmi_5V_regulator = regulator_get(NULL, "vdd_5v_hdmi");	
/*	hdmi_5V_regulator = regulator_get(NULL, "s5p-tvout");	
	if (IS_ERR(hdmi_5V_regulator)) 
	{		
		printk(KERN_ERR "failed to get resource %s\n", "vdd_5v_hdmi");		
		return PTR_ERR(hdmi_5V_regulator);	
	}	
	regulator_enable(hdmi_5V_regulator); */
...
static int s5p_tv_remove(struct platform_device *pdev)
{
	...
	/* ldo6 regulator off */	
	//regulator_disable(hdmi_11_regulator);	
//	regulator_put(hdmi_11_regulator);
	/* ldo7 regulator off */	
	//regulator_disable(hdmi_33_regulator);	
//	regulator_put(hdmi_33_regulator);	

//	regulator_disable(hdmi_5V_regulator);	
//	regulator_put(hdmi_5V_regulator);
